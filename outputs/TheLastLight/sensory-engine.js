(() => {
  ...
  const CAPTIONS = Object.freeze({
    step: '낮은 발소리가 젖은 길 위에 번진다.',
    travel: '바람과 파도가 이동 방향을 따라 흐른다.',
    choice: '짧은 종소리가 선택을 받아 적는다.',
    clue: '맑은 유리음이 새로운 기록을 알린다.',
    solve: '겹친 음들이 하나의 화음으로 풀린다.',
    wrong: '거친 잡음이 낮게 끊어진다.',
    tone: '신호음이 짧게 울린다.',
    signal: '먼 바다에서 무전 신호가 되돌아온다.',
    bell: '물속 종이 긴 잔향을 남긴다.',
    wave: '큰 파도가 선체 아래를 스친다.',
    impact: '둔탁한 충격음과 함께 바닥이 떨린다.',
    thunder: '먼 천둥이 폭풍 속에서 갈라진다.',
    page: '종이가 조용히 넘어간다.',
    finale: '마지막 빛의 선율이 천천히 떠오른다.',
  });
